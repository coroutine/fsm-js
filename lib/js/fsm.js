...
FSM = (function() {
  ...
  FSM.prototype.events = null;

  FSM.prototype.callbacks = null;
  ...
  FSM.prototype.event = function(name, options) {
    ...
    from = transition['from'] === FSM.WILDCARD ? this.states : _([transition['from']]).flatten();
    to = transition['to'];
  ...
  FSM.prototype.eventOptionsAreValid = function(options) {
    var from, fromHasElements, fromInStates, fromIsWildcard, fromStates, to, toInStates, toIsString, transition;
    transition = options != null ? options['transition'] : void 0;
    from = transition != null ? transition['from'] : void 0;
    fromIsWildcard = from === FSM.WILDCARD;
    fromStates = _(_([transition != null ? transition['from'] : void 0]).flatten()).compact();
    fromHasElements = !!fromStates.length;
    fromInStates = _(this.states).intersection(fromStates).length === fromStates.length;
    to = transition != null ? transition['to'] : void 0;
    toIsString = _(to).isString();
    toInStates = _(this.states).contains(to);
    return !!(options && transition && (fromIsWildcard || (fromHasElements && fromInStates)) && to && toIsString && toInStates);
  };
